...
const spiralConfigs = [
  { startX: -300, startY: 100, startRotate: -25, startScale: 0.6 },
  { startX: 300, startY: 150, startRotate: 20, startScale: 0.6 },
  { startX: -250, startY: 200, startRotate: -15, startScale: 0.6 },
  { startX: 250, startY: 250, startRotate: 25, startScale: 0.6 },
];

function SpiralCarCard({ car, index }: { car: typeof cars[0]; index: number }) {
  const cardRef = useRef<HTMLDivElement>(null);
  const config = spiralConfigs[index % spiralConfigs.length];
  ...
  // Transform values for spiral effect
  const x = useTransform(scrollYProgress, [0, 1], [config.startX, 0]);
  const y = useTransform(scrollYProgress, [0, 1], [config.startY, 0]);
  const rotate = useTransform(scrollYProgress, [0, 1], [config.startRotate, 0]);
  const scale = useTransform(scrollYProgress, [0, 1], [config.startScale, 1]);
  ...
  return (
    ...
    >
      <Link href={`/occasions/${car.id}`} className="group block">
        <div className="bg-[#1a1a1a] rounded-3xl overflow-hidden border border-white/10 hover:border-[#c8102e]/50 transition-all duration-500 hover:shadow-2xl hover:shadow-[#c8102e]/10">
          {/* Image Container */}
          <div className="relative aspect-[16/10] overflow-hidden bg-[#0d0d0d]">
            ...
            {/* Year Badge */}
            <div className="absolute top-4 left-4 bg-[#c8102e] text-white px-3 py-1 rounded-full text-sm font-semibold">
              {car.bouwjaar}
            </div>
          </div>
          ...
          <div className="p-6 lg:p-8">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <h3 className="text-2xl lg:text-3xl font-bold text-white group-hover:text-[#c8102e] transition-colors">
                  {car.merk} {car.model}
                </h3>
                <p className="text-white/50 text-lg">{car.variant}</p>
              </div>
              <div className="text-right">
                <p className="text-3xl lg:text-4xl font-bold text-[#c8102e]">
            ...
            <div className="grid grid-cols-3 gap-4 py-4 border-t border-white/10">
              <div className="text-center">
                <p className="text-white/40 text-sm mb-1">Km-stand</p>
                <p className="text-white font-semibold">{car.kilometerstand.toLocaleString('nl-NL')}</p>
              </div>
              <div className="text-center border-x border-white/10">
                <p className="text-white/40 text-sm mb-1">Brandstof</p>
                <p className="text-white font-semibold">{car.brandstof}</p>
              </div>
              <div className="text-center">
                <p className="text-white/40 text-sm mb-1">Transmissie</p>
                <p className="text-white font-semibold">{car.transmissie}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4">
              ...
              <span className="flex items-center gap-2 text-[#c8102e] font-semibold group-hover:gap-3 transition-all">
...
function SpiralCarsSection() {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative py-20 lg:py-32 bg-[#0a0a0a] overflow-hidden min-h-[200vh]"
    >
      {/* Animated Background */}
      <motion.div 
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-lg lg:text-xl text-white/50 max-w-2xl mx-auto">
            Scroll naar beneden en ontdek onze meest exclusieve occasions. 
            Elke auto is zorgvuldig geselecteerd en gecontroleerd.
          </p>
        </motion.div>

        {/* Spiral Cars Container */}
        <div className="space-y-16 lg:space-y-24">
          {featuredCars.map((car, index) => (
            <SpiralCarCard key={car.id} car={car} index={index} />
          ))}
        </div>
        ...
        >
          <Link
            href="/occasions"
            className="group inline-flex items-center gap-3 bg-[#c8102e] hover:bg-[#a00d24] text-white px-10 py-5 rounded-2xl font-semibold text-lg transition-all duration-300 hover:shadow-2xl hover:shadow-[#c8102e]/25"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, index) => (
            ...
            >
              ...
              {service.price && (
                <div className="mb-6">
                  <span className="text-2xl font-bold text-[#c8102e]">€{service.price}</span>
                  <span className="text-white/40 text-sm"> vanaf</span>
                </div>
              )}
              <Link href="/onderhoud" className="inline-flex items-center gap-2 text-white font-semibold group-hover:text-[#c8102e] transition-colors">
                Meer info <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
            </motion.div>
          ))}
        </div>
      ...
      <div className="relative">
        ...
        >
          {reviewsMulti.map((review, index) => (
            <div 
              key={`${review.id}-${index}`}
              className="flex-shrink-0 w-[350px] bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] border border-white/10 rounded-2xl p-6"
            >
