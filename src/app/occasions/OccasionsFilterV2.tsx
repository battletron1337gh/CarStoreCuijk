...
interface OccasionsFilterProps {
  cars: Car[];
}

function OccasionsFilterContent({ cars: initialCars }: OccasionsFilterProps) {
  ...
  // Client-side refresh from public JSON snapshot
  useEffect(() => {
    const refreshCars = async () => {
      try {
        setIsRefreshing(true);
        const res = await fetch('/vwe-voorraad-live.json', { cache: 'no-store' });
        if (!res.ok) return;
        ...
        console.error('Failed to refresh occasions:', err);
      } finally {
    ...
    const interval = setInterval(refreshCars, 5 * 60 * 1000);
  ...
  if (cars.length === 0) {
    return (
      <div className="text-center py-16 bg-[#1a1a1a] rounded-2xl border border-white/10">
        ...
        <p className="text-white/50 mb-4">Er zijn momenteel geen occasions beschikbaar.</p>
      </div>
    );
  }
...
export default function OccasionsFilterV2(props: OccasionsFilterProps) {
  return (
    <Suspense fallback={<FilterLoading />}>
      <OccasionsFilterContent {...props} />
    </Suspense>
  );
}
